...
def final_decision_plot(df, show=False, block=False, trafoD_bins = False, bin_number = 15):
    ...
    plt.hist(df_sig['bin_scaled'].tolist(),
         bins=bins,
         weights=(df_sig['post_fit_weight']*20).tolist(),
         range=plot_range,
         rwidth=1,
         histtype = 'step',
         linewidth=2,
         color='#FF0000',
         label=r'VH $\rightarrow$ Vbb x 20',
         edgecolor='#FF0000')
    ...
    axes = plt.gca()
    axes.set_ylim([0.9,110000])
    axes.set_xlim([-1,1])
    x = [-1,-0.8,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.8,1]
    plt.xticks(x, x,fontweight = 'normal',fontsize = 16)
    y = [1,10,r"10$^{2}$",r"10$^{3}$",r"10$^{4}$",r"10$^{5}$"]
    yi = [1,10,100,1000,10000,100000]
