...
def final_decision_plot(df, show=False, block=False, trafoD_bins = False, bin_number = 15):
    ...
    if (nJets == 2)|(nJets=='2'):
        y = [10,r"10$^{2}$",r"10$^{3}$",r"10$^{4}$"]
        yi = [10,100,1000,10000]
        axes.set_ylim([8,11000])
    else:
        y = [10,r"10$^{2}$",r"10$^{3}$",r"10$^{4}$",r"10$^{5}$"]
        yi = [10,100,1000,10000,100000]
        axes.set_ylim([8,110000])
    ...
    an5 = axes.annotate("$m_{bb} \geq$ 150 GeV", xy=(0.05,0.91), xycoords=axes.transAxes, textcoords=offset_from,fontstyle = 'italic',fontsize = 12)
    
    offset_from = OffsetFrom(an5, (0, -1.2))
    an6 = axes.annotate("$m_{top} \leq$ 225 GeV", xy=(0.05,0.91), xycoords=axes.transAxes, textcoords=offset_from,fontstyle = 'italic',fontsize = 12)
...
def setBinCategory(df,bins):
    ...
    step = 0.1  #step between midpoints
    midpoint = -0.95    #Initial midpoint
