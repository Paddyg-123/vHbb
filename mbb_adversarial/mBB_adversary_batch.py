...
def main():
    ...
    # Prepare data
    df_train = pd.read_csv('ADV_'+str(nJets)+'jet_batch_'+train+'.csv', index_col=0)
    df_test = pd.read_csv('ADV_'+str(nJets)+'jet_batch_'+test+'.csv', index_col=0)
...
def test_classifier(D,df,lam,stage,test):    #tests classifier model and saves results
    ...
    df.to_csv(path_or_buf='adv_results/3jet_'+test+'_batch_'+stage+'_'+str(lam)+'.csv')
